...
module.exports = function(action, options, callback) {
	...
	async.series([
		...
		function loadState(callback) {
			if (loggingEnabled) Log.write('Loading current state... ');
		...
		function ensureLocalStateWriteable(callback) {
			...
			if (loggingEnabled) Log.write('Ensuring local state writable... ');
		...
		function ensureGlobalStateWriteable(callback) {
			if (options.dryRun) return callback();
			if (loggingEnabled) Log.write('Ensuring global state writable... ');
		...
		function executePlan(callback) {
			console.log(chalk.bold('[begin migration]'));
		...
		if (migrationStarted) console.log(chalk.bold('[end migration]'));
		async.series([
			function releaseLock(callback) {
				if (!lockAcquired) return callback();
				if (loggingEnabled) Log.write('Releasing lock... ');
			...
			async.series([
				...
				if (err) {
					...
					if (!options.dryRun) {
						console.log(chalk.red('Migration job failed after ' + duration));
					}
					process.exit(1);
				} else {
					if (!options.dryRun) {
						console.log('');
						console.log(chalk.green('Migration job succeeded after ' + duration));
					}
					process.exit(0);
				}
			})
